...
{
    var today = new Date();
    today.setHours(today.getHours() + zone );
    return today;
}

handlers.sendMailToPlayers = function (args,context) {
    var mailKey = "mail";
    
    if(!args.mail.hasOwnProperty("id") || args.mail.id == "")
    {
        args.mail.id = "";
        var today = getTime(7);
        args.mail.id += makeid(4) + "|" + today.getMonth().toString() + "-" + today.getDate().toString() + "-" + today.getFullYear().toString();
    }
    
    if(!args.mail.hasOwnProperty("start"))
    {
        args.mail.start = printDate(getTime(7));
    }
    
    if(!args.mail.hasOwnProperty("expire"))
    {
        args.mail.expire = printDate(getTime(7+24*90));
    }
    
    
    
    var playerIds = args.ids;
    ...
    for(i = 0; i < playerIds.length; i++)
    {
        ...
        try {
            var playerData = server.GetUserReadOnlyData({"PlayFabId" : currentPlayerId, "Keys" : [mailKey]});
            ...
            var obj = {id: args.mail.id, recipient: "all", title: args.mail.title, message: args.mail.message,items: args.mail.items,start:args.mail.start , expire: args.mail.expire}; 
            var duplicateId = false;
            if(playerData.Data.hasOwnProperty(mailKey))
            {
                previousState = JSON.parse(playerData.Data[mailKey].Value);
                for(ii = 0; ii < previousState.length; ii++)
                {
                    if(previousState[ii].id == obj.id)
                    {
                        duplicateId = true;
                        previousState[ii] = obj;
                    }
                }
            }
            if(!duplicateId) previousState.push(obj);
            
            writeToServer[mailKey] = JSON.stringify (previousState);
...
handlers.updateRaidBoss = function (args, context){
    var request = {
        PlayFabId: currentPlayerId, Statistics: [{
                StatisticName: "Raid Boss",
                Value: args.score
            }]
    };
    // The pre-defined "server" object has functions corresponding to each PlayFab server API 
